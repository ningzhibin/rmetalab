...
#' a ggplot2 GUI plot or describe a column from a DF, grouping is supported
#'
#' forked from ggplotgui package (https://github.com/gertstulp/ggplotgui), with slight modification, for the convenience of being incorporated into Rmarkdonwn document in interactive metapreport
#' The orginal code did not use any module, therefore a bit complicated to jump back and forth, but it works.
#'
#' @param dataset optional, a data.frame to be plotted, if not provided, the data of iris is used for testing
#'
#'
#' @export
#'
...
ggplot2_shiny <- function(dataset = NA){

  library(ggplotly)

  ui <- fluidPage(headerPanel("ggplot GUI"),
                  sidebarPanel(width = 3,
                               conditionalPanel(condition = "input.tabs=='Data upload'",h4("Data upload"),
                                                radioButtons("data_input", "", choices = if (is.data.frame(dataset)){
                                                                                                                    list(`Load sample data` = 1,
                                                                                                                          `Upload text file` = 2,
                                                                                                                          `Paste data` = 3,
                                                                                                                          `Data passed through R environment` = 4)} else {
                                                                                                              list(`Load sample data` = 1,
                                                                                                                   `Upload file` = 2,
                                                                                                                   `Paste data` = 3)},
                                                                                                   selected = if (is.data.frame(dataset)) 4 else 1),
                                                conditionalPanel(condition = "input.data_input=='1'", h5("dataset 'mpg' from library(ggplot2) loaded")),
                                                conditionalPanel(condition = "input.data_input=='2'", h5("Upload file: "), fileInput("upload", "", multiple = FALSE), selectInput("file_type", "Type of file:", list(`text (csv)` = "text", Excel = "Excel", SPSS = "SPSS", Stata = "Stata", SAS = "SAS"),  selected = "text"),
                                                                conditionalPanel(condition = "input.file_type=='text'", selectInput("upload_delim", "Delimiter:", list(Semicolon = ";", Tab = "\t", Comma = ",", Space = " "), selected = "Semicolon")),
                                                                actionButton("submit_datafile_button", "Submit datafile")),
                                                conditionalPanel(condition = "input.data_input=='3'", h5("Paste data below:"), tags$textarea(id = "data_paste", placeholder = "Add data here", rows = 10, cols = 20, ""),
                                                                actionButton("submit_data_button", "Submit data"),
                                                                selectInput("text_delim", "Delimiter:", list(Semicolon = ";",Tab = "\t", Comma = ",", Space = " "), selected = "Semicolon"))),
                               conditionalPanel(condition = "input.tabs=='ggplot' || input.tabs=='Plotly' ||\n input.tabs=='R-code'", h4("Create visualization"), selectInput(inputId = "Type",label = "Type of graph:", choices = c("Boxplot",
                                                                                                                                                         "Density", "Dot + Error", "Dotplot", "Histogram",
                                                                                                                                                         "Scatter", "Violin"), selected = "Violin"),
                                                                           ...
                                                                           conditionalPanel(condition = "input.Type == 'Boxplot' || input.Type == 'Violin' ||\n          input.Type == 'Dot + Error'",
                                                                                            checkboxInput(inputId = "jitter", label = strong("Show data points (jittered)"),
                                                                                                          value = FALSE)), conditionalPanel(condition = "input.Type == 'Boxplot'",
                                                                                                                                            checkboxInput(inputId = "notch", label = strong("Notched box plot"),
                                                                                                                                                          value = FALSE)), conditionalPanel(condition = "input.Type == 'Density' || input.Type == 'Histogram'",
                                                                                                                                                                                            sliderInput("alpha", "Opacity:", min = 0, max = 1,
                                                                                                                                                                                                        value = 0.8)), conditionalPanel(condition = "input.Type == 'Histogram' || input.Type=='Dotplot'",
                                                                                                                                                                                                                                        numericInput("binwidth", "Binwidth:", value = 1)),
                                                                           conditionalPanel(condition = "input.Type == 'Dotplot'",
                                                                                            selectInput("dot_dir", "Direction stack:", choices = c("up",
                                                                                                                                                   "down", "center", "centerwhole"), selected = "up")),
                                                                           conditionalPanel(condition = "input.Type == 'Density' || input.Type == 'Violin'",
                                                                                            sliderInput(inputId = "adj_bw", label = "Bandwidth adjustment:",
                                                                                                        min = 0.01, max = 2, value = 1, step = 0.1)),
                                                                           conditionalPanel(condition = "input.Type == 'Scatter'",
                                                                                            checkboxInput(inputId = "line", label = strong("Show regression line"),
                                                                                                          value = FALSE), conditionalPanel(condition = "input.line == true",
                                                                                                                                           selectInput("smooth", "Smoothening function",
                                                                                                                                                       choices = c("lm", "loess", "gam"))), conditionalPanel(condition = "input.line == true",
                                                                                                                                                                                                             checkboxInput(inputId = "se", label = strong("Show confidence interval"),
                                                                                                                                                                                                                           value = FALSE))), conditionalPanel(condition = "input.Type == 'Dot + Error'",
                                                                                                                                                                                                                                                              selectInput("CI", "Confidence Interval:", choices = c(`68% (1 SE)` = 1,
                                                                                                                                                                                                                                                                                                                    `90%` = 1.645, `95%` = 1.96, `99%` = 2.575),
                                                                                                                                                                                                                                                                          selected = 1.96))), conditionalPanel(condition = "input.tabs=='Info'",
                                                                                                                                                                                                                                                                                                               h4("Info"))), h6("For more info see the 'Info'-tab or visit",
                                                                                                                                                                                                                                                                                                                                a("https://github.com/gertstulp/ggplotgui", href = "https://github.com/gertstulp/ggplotgui")),
                  mainPanel(width = 6, tabsetPanel(type = "tabs", selected = "ggplot", tabPanel("Data upload",
                                                                                                dataTableOutput("out_table")), tabPanel("ggplot",
                                                                                                                                        mainPanel(downloadButton("download_plot_PDF", "Download pdf of figure"),
                                                                                                                                                  plotOutput("out_ggplot"))), tabPanel("Plotly",
                                                                                                                                                                                       plotlyOutput("out_plotly")), tabPanel("R-code", verbatimTextOutput("out_r_code")),
                                                   tabPanel("Info", h3("Background"), p(a("R", href = "https://www.r-project.org/"),
                                                            ...
                                                            h3("App info"), p("This application was built in ",
                                                                              ...
                                                                              ",", a("haven", href = "http://haven.tidyverse.org/"),
                                                                              ", and", a("RColorBrewer.", href = "http://stringr.tidyverse.org/")),
                                                            p("This application was created by ", a("Gert Stulp",
                                                                                                    href = "http://www.gertstulp.com/"), ". Please do report bugs and send feature requests to ",
                                                              a("g.stulp[at]rug.nl", href = "mailto:g.stulp@rug.nl"),
                                                              ". Visit ", a("https://github.com/gertstulp/ggplotgui",
                                                                            href = "https://github.com/gertstulp/ggplotgui"),
                                                              "for further description and code."), h3("Acknowledgements"),
                                                            p("Thanks to Wilmer Joling for setting up the ",
                                                              ...
                                                              ". Thanks to ", a("Hadley Wicham", href = "http://hadley.nz/"),
                                                   ...
                                                   id = "tabs")), conditionalPanel(condition = "input.tabs=='ggplot' || input.tabs=='Plotly' ||\n                    input.tabs=='R-code'",
                                                                                   sidebarPanel(width = 3, h4("Change aesthetics"),
                                                                                                tabsetPanel(tabPanel("Text", checkboxInput(inputId = "label_axes",
                                                                                                                                           label = strong("Change labels axes"), value = FALSE),
                                                                                                                     ...
                                                                                                                     checkboxInput(inputId = "add_title", label = strong("Add title"),
                                                                                                                                   value = FALSE), conditionalPanel(condition = "input.add_title == true",
                                                                                                                                                                    textInput("title", "Title:", value = "Title")),
                                                                                                                     checkboxInput(inputId = "adj_fnt_sz", label = strong("Change font size"),
                                                                                                                                   value = FALSE), conditionalPanel(condition = "input.adj_fnt_sz == true",
                                                                                                                                                                    numericInput("fnt_sz_ttl", "Size axis titles:",
                                                                                                                                                                                 value = 12), numericInput("fnt_sz_ax",
                                                                                                                                                                                                           "Size axis labels:", value = 10)), checkboxInput(inputId = "rot_txt",
                                                                                                                                                                                                                                                            label = strong("Rotate text x-axis"), value = FALSE),
                                                                                                                     checkboxInput(inputId = "adj_fnt", label = strong("Change font"),
                                                                                                                                   value = FALSE), conditionalPanel(condition = "input.adj_fnt == true",
                                                                                                            ...
                                                                                                            tabPanel("Theme", conditionalPanel(condition = "input.group != '.'",
                                                                                                                                               checkboxInput(inputId = "adj_col", label = strong("Change colours"),
                                                                                                                                                             value = FALSE), conditionalPanel(condition = "input.adj_col",
                                                                                                                                                                                              selectInput(inputId = "palet", label = strong("Select palette"),
                                                                                                                                                                                                          choices = list(Qualitative = c("Accent",
                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                         "Set1", "Set2", "Set3"), Diverging = c("BrBG",
                                                                                                                                                                                                                                                                                "PiYG", "PRGn", "PuOr", "RdBu", "RdGy",
                                                                                                                                                                                                                                                                                "RdYlBu", "RdYlGn", "Spectral"), Sequential = c("Blues",
                                                                                                                                                                                                                                                                                                                                "BuGn", "BuPu", "GnBu", "Greens", "Greys",
                                                                                                                                                                                                                                                                                                                                "Oranges", "OrRd", "PuBu", "PuBuGn",
                                                                                                                                                                                                                                                                                                                                "PuRd", "Purples", "RdPu", "Reds",
                                                                                                                                                                                                                                                                                                                                "YlGn", "YlGnBu", "YlOrBr", "YlOrRd")),
                                                                                                                                                                                                          selected = "set1"))), conditionalPanel(condition = "input.jitter",
                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                        textInput("col_jitter", "Colour (name or RGB):",
                                                                                                                                                                                                                                                                                                  value = "black"), numericInput("size_jitter",
                                                                                                                                                                                                                                                                                                                                 "Size:", value = 1), sliderInput("opac_jitter",
                                                                                                                                                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                                                                                                                                                  step = 0.01), sliderInput("width_jitter",
                                                                                                                                                                                                                                                                                                                                                                                            "Width jitter:", min = 0, max = 0.5,
                                                                                                                                                                                                                                                                                                                                                                                            value = 0.25, step = 0.01))), checkboxInput("adj_grd",
                                                                                                                                                                                                                                                                                                                                                                                                                                        strong("Remove gridlines"), FALSE), conditionalPanel(condition = "input.adj_grd",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             checkboxInput("grd_maj", strong("Remove major gridlines"),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           FALSE), checkboxInput("grd_min", strong("Remove minor gridlines"),
                                                                                                            ...
                                                                                                            tabPanel("Legend", conditionalPanel(condition = "input.group != '.'",
                                                                                                                                                radioButtons(inputId = "adj_leg", label = NULL,
                                                                                                                                                             choices = c("Keep legend as it is", "Remove legend",
                                                                                                                                                                         "Change legend"), selected = "Keep legend as it is"),
                                                                                                            ...
                                                                                                            tabPanel("Size", checkboxInput("fig_size",
                                                                                                                     ...
                                                                                                                     checkboxInput("fig_size_download", strong("Adjust plot size for download"),
                                                                                                                                   FALSE), conditionalPanel(condition = "input.fig_size_download",
                                                                                                                                                            numericInput("fig_height_download", "Plot height (in cm):",
                                                                                                                                                                         value = 14), numericInput("fig_width_download",
                                                                                                                                                                                                   "Plot width (in cm):", value = 14)))))))
  server <- function(input, output, session) {
    observe({
      nms <- names(df_shiny())
      nms_cont <- names(Filter(function(x) is.integer(x) ||
                                 is.numeric(x) || is.double(x), df_shiny()))
      nms_fact <- names(Filter(function(x) is.factor(x) ||
                                 is.logical(x) || is.character(x), df_shiny()))
      ...
      updateSelectInput(session, "x_var", choices = c(`No x-var` = "' '",
                                                      nms))
      ...
      if (input$data_input == 1) {
        ...
        if (is.null(input$upload)) {
          ...
          isolate({
            if (input$file_type == "text") {
              data <- read_delim(file_in$datapath, delim = input$text_delim,
                                 col_names = TRUE)
            }
        ...
        if (input$data_paste == "") {
          ...
          if (input$submit_data_button == 0) {
            ...
            isolate({
              data <- read_delim(input$data_paste, delim = input$text_delim,
                                 col_names = TRUE)
            })
          }
        }
    ...
    string_code <- reactive({
      ...
      if (gg_fil || input$Type == "Scatter")
        jitt <- FALSE
      else jitt <- input$jitter
      p <- paste("ggplot(df, aes(", if (gg_x_y) {
        "x = input$y_var"
      }
      else {
        ...
        paste("geom_histogram(position = 'identity', alpha = input$alpha, ",
              "binwidth = input$binwidth)", sep = ""), if (input$Type ==
                                                           "Density")
                paste("geom_density(position = 'identity', alpha = input$alpha, ",
                      "adjust = input$adj_bw)", sep = ""), if (input$Type ==
                                                               "Boxplot")
                        "geom_boxplot(notch = input$notch)", if (input$Type ==
                                                                 "Violin")
                          "geom_violin(adjust = input$adj_bw)", if (input$Type ==
                                                                    "Dotplot")
                            paste("geom_dotplot(binaxis = 'y', binwidth = input$binwidth, ",
                                  "stackdir = 'input$dot_dir')", sep = ""), if (input$Type ==
                                                                                "Dot + Error")
                                    paste("geom_point(stat = 'summary', fun.y = 'mean') +\n  ",
                                          "geom_errorbar(stat = 'summary', fun.data = 'mean_se', ",
                                          "\n                width=0, fun.args = list(mult = input$CI))",
                                          sep = ""), if (input$Type == "Scatter")
                                            "geom_point()", if (input$Type == "Scatter" &&
                                                                input$line)
      ...
      if (input$adj_fnt_sz || input$adj_fnt || input$rot_txt ||
          input$adj_leg != "Keep legend as it is" || input$adj_grd) {
      ...
      p <- stringr::str_replace_all(p, c(`input\\$y_var` = input$y_var,
    ...
    output$out_table <- renderDataTable(df_shiny())
    width <- reactive({
      input$fig_width
    })
    height <- reactive({
    ...
    output$out_r_code <- renderText({
      ...
      paste("## You can use the below code to generate the graph.\n",
            ...
            "ggsave('my_graph.pdf', graph, width = ", width_download(),
            ", height = ", height_download(), ", units = 'cm')",
            sep = "")
    })
    output$download_plot_PDF <- downloadHandler(filename <- function() {
      paste("Figure_ggplotGUI_", Sys.time(), ".pdf", sep = "")
    }, content <- function(file) {
      df <- df_shiny()
      p <- eval(parse(text = string_code()))
      ggsave(file, p, width = width_download(), height = height_download(),
             units = "cm")
